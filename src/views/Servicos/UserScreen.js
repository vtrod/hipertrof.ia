...
const UserScreenContent = ({ navigation }) => {
  ...
  const handleGenerateTraining = async () => {
    ...
    if (!weight || !height || !sex || !fitnessLevel || !selectedAge || !trainingDays || !trainingHours) {
      alert('Por favor, preencha todos os campos.');
      return;
    };

    const prompt = `Crie uma planilha de treinos para uma pessoa que tenha o peso ${formData.weight}, a altura ${formData.height}, seja do sexo ${formData.sex}, possua um físico ${formData.fitnessLevel}, de idade ${formData.selectedAge}, possa treinar ${formData.trainingDays} na semana e ${formData.trainingHours} horas por dia, com o histórico de lesão de ${formData.injuryHistory}, treina no ambiente ${formData.equipmentAvailable}, e tenha limitação de exercício ${formData.exercisePreferences}.`;
  ...
  try {
    ...
    }, 
    {
      headers: {
        ...
        'Authorization': 'Bearer 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', // Sua API key
      },
    });

    // resposta da API 
    Alert.alert('Treino gerado com sucesso:', JSON.stringify(response.data));
  } catch (error) {
